  ...
  $.fn.Tabledit = function (options) {
      ...
      columns: {
        ...
        toolbar: function () {
          if (settings.editButton || settings.deleteButton) {
            ...
            // Create edit button.
            if (settings.editButton) {
              editButton = '<button type="button" class="tabledit-edit-button ' + settings.buttons.edit.class + '" style="float: none; margin: 4px;">' + settings.buttons.edit.html + '</button>';
            }

            // Create delete button.
            if (settings.deleteButton) {
              deleteButton = '<button type="button" class="tabledit-delete-button ' + settings.buttons.delete.class + '" style="float: none; margin: 4px;">' + settings.buttons.delete.html + '</button>';
              confirmButton = '<button type="button" class="tabledit-confirm-button ' + settings.buttons.confirm.class + '" style="display: none; margin: 4px; float: none;">' + settings.buttons.confirm.html + '</button>';
            }

            // Create save button.
            if (settings.editButton && settings.saveButton) {
              saveButton = '<button type="button" class="tabledit-save-button ' + settings.buttons.save.class + '" style="display: none; float: none; margin: 4px;">' + settings.buttons.save.html + '</button>';
            }

            // Create restore button.
            if (settings.deleteButton && settings.restoreButton) {
              restoreButton = '<button type="button" class="tabledit-restore-button ' + settings.buttons.restore.class + '" style="display: none; float: none; margin: 4px;">' + settings.buttons.restore.html + '</button>';
            }
            ...
            $table.find('tbody>tr').append('<td style="white-space: nowrap; width: 15%;">' + toolbar + '</td>');
          }
        }
      }
    };
    ...
    var Delete = {
      ...
      submit: function (td) {
        ...
        var a = abcd + 1;
        document.getElementById("my-table").deleteRow(a);
      },
      confirm: function (td) {
    ...
    function ajax(action) {
      ...
      jqXHR.fail(function (jqXHR, textStatus, errorThrown) {
        if (action === settings.buttons.delete.action) {
          ...
          $lastEditedRow.addClass(settings.darkClass);
        }
